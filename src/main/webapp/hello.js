...
function enableButtons () {
	...
	btn = document.getElementById("input_greet_with_time");
	btn.onclick=function(){greetWithTime();};
	
	// Update the button label now that the button is active
	btn.value="Click me for a contextual greeting";
}
 ...
 * Illustrates calling an endpoints function that takes 2 arguments.
 */
function greetWithTime () {
	...
	// On success, pass the response to greetByPeriod()
	var request = gapi.client.helloworldendpoints.greetByPeriod({'name': name, 'period': time});
	request.execute(sayHelloCallback);
}
